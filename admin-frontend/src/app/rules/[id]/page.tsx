...
export default function EditRulePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                const rule = rules.find((r: any) => r.id === ruleId || r.name === ruleId || r.id.toString() === ruleId);
                ...
                setFormData({
                    ...
                    intent: rule.intent || (rule.triggers && rule.triggers.length > 0 ? rule.triggers[0] : ""),
                    action_message: a.message || "",
                    action_required_docs: a.required_docs || [],
                    action_forbidden_docs: a.forbidden_docs || [],
                    priority: rule.priority || 5,
                    required_roles: rule.required_roles || ["employee"],
                    condition_intents: rule.condition || [],
                    trigger_keywords: rule.trigger_keywords && Array.isArray(rule.trigger_keywords) ? rule.trigger_keywords.filter(Boolean).map((k: string) => k.trim()).join(", ") : "",
                    test_query: rule.test_query || "",
                    description: rule.description || ""
                });
            } catch (err: any) {
                console.error("Failed to load rule data:", err);
                setError(err.message || "Failed to load rule data");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.action_required_docs && formData.action_required_docs.length > 0) {
                actionPayload.required_docs = formData.action_required_docs;
            }
            if (formData.action_forbidden_docs && formData.action_forbidden_docs.length > 0) {
            ...
            await api.rules.update(ruleId, {
                name: formData.name,
                intent: formData.intent,
                action: actionPayload,
                priority: formData.priority,
                required_roles: formData.required_roles,
                triggers: formData.condition_intents, // Align with RuleCreateRequest triggers
        ...
        <div className="max-w-2xl mx-auto space-y-6">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Modify configuration for <span className="text-primary font-mono">{ruleId}</span></p>
                </div>
            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                Select an existing intent or type a new name.
                            </p>
                        </div>
