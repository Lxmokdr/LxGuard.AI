...
```powershell
curl -s -X POST "$VENDOR_API_URL/admin/auth/register-first-admin?email=admin@lxguard.ai&password=CHANGE_THIS"
...
$response = curl -s -X POST $VENDOR_API_URL/admin/auth/login `
  -H "Content-Type: application/x-www-form-urlencoded" `
  -d "username=admin@lxguard.ai&password=CHANGE_THIS" | ConvertFrom-Json
...
```powershell
$customerBody = '{"name": "Client Organisation Name", "contact_email": "it@clientorg.com", "organization_type": "enterprise"}'

$customer = curl -s -X POST $VENDOR_API_URL/admin/customers `
  -H "Authorization: Bearer $VENDOR_TOKEN" `
  -H "Content-Type: application/json" `
  -d $customerBody | ConvertFrom-Json
...
```powershell
$licenseBody = "{`"customer_id`": `"$CUSTOMER_ID`", `"max_instances`": 3, `"expires_at`": `"2027-12-31T23:59:59`"}"

$license = curl -s -X POST $VENDOR_API_URL/admin/licenses `
  -H "Authorization: Bearer $VENDOR_TOKEN" `
  -H "Content-Type: application/json" `
  -d $licenseBody | ConvertFrom-Json
...
```powershell
$licenses = curl -s -H "Authorization: Bearer $VENDOR_TOKEN" `
  $VENDOR_API_URL/admin/licenses | ConvertFrom-Json
...
$instances = curl -s -H "Authorization: Bearer $VENDOR_TOKEN" `
  $VENDOR_API_URL/admin/instances | ConvertFrom-Json
...
```powershell
$metrics = curl -s -H "Authorization: Bearer $VENDOR_TOKEN" `
  "$VENDOR_API_URL/admin/metrics?limit=10" | ConvertFrom-Json
